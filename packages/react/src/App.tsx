...
// Props are handled by PromQLPlayground wrapper, App receives via context

function App() {
  const { scenarios } = usePlayground();
  ...
  // Initialize with first scenario when scenarios load
  useEffect(() => {
    if (scenarios.length > 0 && !selectedScenario) {
      const firstScenario = scenarios[0];
      setSelectedScenario(firstScenario);
      if (firstScenario.sampleQueries.length > 0) {
        setQuery(firstScenario.sampleQueries[0].query);
      }
    }
  ...
  useEffect(() => {
    if (query && !executeQuery.data && !executeQuery.isPending && selectedScenario) {
      executeQuery.mutate({ query, timeRange });
    }
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <div style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
                ...
                {activeResultTab === 'explain' && selectedScenario && (
